...
import static com.yizhuoyan.shidao.common.dao.support.JDBCUtil.generateInsertSQL;
import static com.yizhuoyan.shidao.common.dao.support.JDBCUtil.generateLikeSql;


public abstract class SingleTableDaoSupport<T> implements CRUDDao<T>, PaginationQueryDao<T>, QueryDao<T>{
...
@Override
public List<T> selectsLike(String key, String likeColumns, String orderBys) throws Exception{
  //be liked columns
  String[] columnArr = likeColumns.split(",");
  //组装wheresql
  Sql sql = Sql.select(generateSelectColumns())
                .from(tableName)
                .whereIf(generateLikeSql(columnArr, key))
                .orderByIf(orderBys);
  // 3传入参数
  if(key!=null){
    key = "%"+key+"%";
    Object[] params = new String[columnArr.length];
    Arrays.fill(params, key);
    return executesQuerySql(sql, this::row2obj, params);
  }else{
...
@Override
public int selectsLike(String key, String columns, String orderBy, int pageNo, int pageSize, List<T> pageData) throws Exception{
  //the total rows
  int total = 0;
  //be liked columns
  ...
  Sql countSql = Sql.selectCount("1")
                     .from(tableName)
                     .whereIf(whereLikeSql);

  Object[] params = null;
  if(key!=null){
    key = "%"+key+"%";
    params = new String[columnArr.length];
    Arrays.fill(params, key);
  }
...
@Override
public List<T> selects(String column, Object value) throws Exception{
  Sql sql = Sql.select(generateSelectColumns())
                .from(tableName)
                .where(column, "=?");
