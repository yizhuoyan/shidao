...
//onload
    ...
    var updateFunctionalityTableTree = function () {
        $tableTreeEL.fancytree({
            ...
            checkbox: true,
            minExpandLevel: 1,
            selectMode: Number(setting.pickMode),
            extensions: ["table"],
            table: {
                checkboxColumnIdx: 0,
                nodeColumnIdx: 1,
            ...
            init: function (evt, ctx, flag) {
                ...
                //选择已选择的节点
                if (setting.select) {
                    var arr = setting.select.split(",");
                    for (var i = arr.length; i-- > 0;) {
                        var node = tree.getNodeByKey(arr[i]);
                        if (node) {
    ...
    var addSelectedConfirmBtnClickHandler = function () {
        $("#confirmPickBtn").click(function () {
            try {
                ...
                if (setting.pickMode === "1") {
                    //单选模式仅返回一个，没有则undefined
                    window.broadcast(setting.broadcastKey, selectedNodes[0]);
                } else {
                    window.broadcast(setting.broadcastKey, selectedNodes);
                }
            } finally {
                //保证一定关闭窗口
                window.close();
